...
- 每轮邮件同步按主号建立一个只读 IMAP 连接，并处理这个主号下的所有已启用隐私邮箱，不会为每个隐私邮箱分别查询 iCloud。普通增量每批最多提交 256 封，提交后立即继续下一批；较小的 UID 数值区间使用有界 `UID SEARCH ... UNSEEN`，较大或稀疏的区间先用少量序号探测定位起点，再用一次最多返回 257 条的 sequence `FETCH (UID FLAGS)` 检查窗口并在本地排除 `\Seen`。只有其中最早的最多 256 封未读邮件会进入后续邮件头/正文 FETCH。候选邮件头按 UID 批量 `BODY.PEEK` 读取并在本地归类，只对各地址最终命中的最新候选读取完整正文。单封邮件的投递头损坏、超限或相互冲突时，只跳过该封不可安全归属的邮件，不会阻塞整个主号的游标；IMAP 命令失败、重复 UID 或邮箱序号视图异常仍会让本批失败并从已提交游标重试。
- 定时同步和后台手动同步都会先取得该主号的账号锁及全局 IMAP 连接额度，随后才开始计算 `ICLOUD_API_SYNC_TIMEOUT`；默认 `10m` 因此是实际执行数据库读取、IMAP 收取和事务提交的完整预算，不会消耗在排队等待上。每条 IMAP 网络命令仍分别受默认 `25s` 的 `ICLOUD_API_IMAP_TIMEOUT` 限制。
