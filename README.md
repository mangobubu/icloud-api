...
这是一个使用 Go、Gin、Vue 3、Element Plus、iCloud IMAP 和 PostgreSQL 构建的轻量收件服务。Vue 管理端可以登记 iCloud 主号，并为主号绑定多个“隐藏邮件地址”（隐私邮箱）；Go API 服务定时通过该主号的同一个 IMAP 收件箱同步未读邮件。每个隐私邮箱拥有独立 API Key，调用者只能读取这个 Key 所属隐私邮箱由主号级未读同步写入的最新数据库快照；可直接访问的紧凑链接会一次性消费最近一小时内的当前快照，完整 Bearer 接口则可重复读取快照。
...
- 邮件同步以主号为单位建立一条只读 IMAP 连接，并使用 PostgreSQL 中保存的 `UIDVALIDITY + last_uid` 游标发现和处理主号 `INBOX` 中的 `UNSEEN` UID；隐私邮箱数量不会转换成同等数量的 iCloud 查询，也不会触发逐隐私邮箱历史快照恢复。直达接口成功消费邮件后，独立后台任务再按主号建立可写 IMAP 连接，把对应 UID 标记为 `\Seen`。
- 管理端会实时显示手动同步和定时自动同步的当前阶段与完成百分比；最近同步失败时先显示简短错误，并提供“点击查看错误日志”入口查看完整错误。
...
App 专用密码只用于后台的 IMAP 密码字段。邮件正文和文件夹始终通过 `imap.mail.me.com:993` 和 TLS 获取：快照同步使用只读 `SELECT`，所有同步路径都按主号级游标只处理 `UNSEEN`；首次同步、游标重置或 `UIDVALIDITY` 变化时，只先确定收件箱最近最多 1024 封邮件的窗口边界，再分批读取该窗口内的未读邮件。邮件头和正文只对未读候选使用 `BODY.PEEK`，不会读取已读邮件正文，也不会设置已读。只有直达接口成功消费后的后台任务才使用可写 `SELECT` 和 `UID STORE` 设置 `\Seen`。该部署假定没有其他邮件客户端或外部流程读邮件/改变 `\Seen`；IMAP 用户名通常是完整的 iCloud 主号邮箱地址。Apple 账户密码不会代替 App 专用密码访问邮件。

### 2. 同步已有隐私邮箱目录

App 专用密码只开放 IMAP 邮件访问，不能读取 Apple 账户中已有的完整“隐藏邮件地址”目录。要自动导入该目录，请在主号详情页点击“同步隐私邮箱”，首次连接时填写 Apple ID、Apple 账户密码，并按提示输入 6 位双重认证验证码。Apple 账户密码和验证码只用于建立受信任的 Apple Web 会话，不写入数据库；服务仅使用主密钥加密保存会话材料。该受信任会话也用于从 Apple 目录永久删除隐私邮箱；会话过期后需要重新登录。

目录同步读取 Apple `premiummailsettings` 服务的 `/v2/hme/list` 完整列表，不会扫描收件箱，也不会只导入曾经收到过邮件的地址。服务会保留列表中的启用和停用地址，并只导入 `forwardToEmail` 与当前主号邮箱匹配的条目；不匹配的条目会被过滤。超过本地启用容量的地址仍会导入，但初始标记为 `disabled`。每个新导入地址都会生成只显示一次的独立邮件 API Key，已有地址保持原 Key、本地启停状态和邮件快照。仅在 Apple 端停用的地址仍会出现在完整目录中并被导入为本地停用状态；管理端的“永久删除”会完成 Apple 端删除，因此不会在后续目录同步中重新出现。

### 3. 自动创建隐私邮箱（可选）

主号详情页的“自动创建隐私邮箱”开关按主号独立生效。开启后，服务以开启时刻为周期锚点，每 60 分钟最多发起 5 次 Apple Hide My Email 创建尝试；每次间隔随机生成且至少 5 分钟，5 段间隔合计恰好 60 分钟。计划保存在数据库，服务重启后继续使用原计划；任务严重迟到时会从当前时间重新排期，不会一次性补发。Apple 会话失效、限流或服务故障只消耗当前计划槽，不会重新发起本次创建。

自动创建的地址会先写入本地主号并生成独立邮件 API Key。原始 Key 使用主密钥的独立加密上下文保存，不会写入日志；地址经 Apple 目录确认后才会启用并进入“待领取”队列。管理员在主号详情页领取后，应下载或保存 Key，再点击确认保存。只有本次领取返回的地址 ID 会被删除，之后新产生的待领取 Key 不会被误删。达到每个主号 1000 个启用隐私邮箱上限后，服务停止该主号的自动创建。

开启前主号必须已有有效的 Apple Web 会话，且 Apple 隐私邮箱转发目标必须是当前主号。每个计划槽都会在远端创建前重新读取 Apple 的 `selectedForwardTo`。从未创建过隐私邮箱的账号若尚未初始化该字段，服务只会在 Apple 目录为空且 `forwardToEmails` 明确包含当前主号时，将默认转发目标设置为当前主号；设置后必须重新读取并确认 `selectedForwardTo` 完全匹配，才会调用 `generate`/`reserve`。已有远端地址、候选不包含当前主号、目标不匹配或回读无法确认时都不会自动修改设置或创建地址，需要在 iCloud 中手动确认。`reserve` 省略转发信息时，服务会先原子保存停用的候选地址、加密 Key 和轮换后的 Apple 会话，再通过远端列表按新地址确认实际转发目标；若目录短暂未显示新地址或确认读取遇到瞬时错误，只会有界重试这一只读查询。重试后仍未确认时，后续计划槽只继续对账同一个候选地址，不会再次调用 `generate` 或 `reserve`。自动创建、手动“同步隐私邮箱”和远端永久删除共用主号操作锁；外部 `POST /api/v1/aliases` 仍只登记已经存在的地址，不会替代自动创建流程。一个隐私邮箱只能登记一次，也只能归属于一个主号。
...
- `postgres_data`：当前全部运行数据，包括主号、隐私邮箱、邮件快照、同步游标、一次性消费记录、待处理的 IMAP `\Seen` 任务、会话和审计记录。
...
# 单个主号取得同步锁和全局 IMAP 连接额度后的实际同步预算；单次 IMAP 操作仍默认 25s
ICLOUD_API_SYNC_TIMEOUT=10m
TZ=Asia/Shanghai
```

从旧版本升级时，如果现有 `.env` 曾显式把 `ICLOUD_API_SYNC_TIMEOUT` 设为 `10s` 或旧默认值 `2m`，请在重新构建或重启容器前改为 `10m`；未显式设置时会自动采用新的 `10m` 默认值。新版本仍会拒绝启动同步预算不足单次 IMAP 操作时限两倍的配置，避免服务启动后所有同步都稳定超时。
...
升级时，原命名卷 `icloud_api_data` 会只读挂载到 `/app/legacy`。如果其中存在 `/app/legacy/icloud-api.db`，且新的 PostgreSQL 业务表仍为空，应用会在启动阶段自动把管理员、后台会话、主号、隐私邮箱、最新邮件快照、审计记录、Apple Web 会话、自动创建计划和待领取 Key 复制到 PostgreSQL。导入使用事务、数据库级互斥锁和完成标记；成功后重复启动不会重复写入。旧 SQLite 文件不存在时会直接跳过。旧版本没有一次性消费历史和 IMAP `\Seen` 队列；升级后已有最新快照初始视为尚未消费，首次符合条件的直达请求可以消费它，新队列初始为空。
...
前端请求和 API 请求都记录在同一个应用日志中。生产部署只运行一个 `icloud-api` 副本；当前主号同步互斥和接口限流保存在进程内，多副本会重复轮询 IMAP、重复尝试处理同一批 `\Seen` 任务，并拆分限流状态。PostgreSQL 由 Compose 单独持久化，不改变这一应用单副本约束。
...
| `ICLOUD_API_IMAP_TIMEOUT` | `25s` | 单次 IMAP 网络操作超时；提高整轮同步预算不会改变此值 |
| `ICLOUD_API_SYNC_TIMEOUT` | `10m` | 单个主号取得账号同步锁和全局 IMAP 连接额度后，一整轮实际同步的预算；等待锁或连接额度不占用这段预算。范围 `10s` 到 `30m`，且至少为 `ICLOUD_API_IMAP_TIMEOUT` 的两倍 |
...
5. 如需持续补充地址，在主号详情页确认 Apple 会话有效后开启“自动创建隐私邮箱”；开关开启后按每小时 5 次、随机且至少 5 分钟间隔的计划运行。
6. 自动创建的完整 API Key 会在主号详情页的“待领取”队列中显示。保存后点击确认；关闭弹窗或离开页面不会清空队列，之后可以再次领取。
7. 使用“同步邮件”检查 IMAP 连接和收件状态；后台也会按轮询间隔自动同步所有已启用主号。手动同步和定时自动同步执行时，页面都会实时显示当前阶段与完成百分比。积压超过单批上限时，页面会提示“已提交一批，仍在追平”，后续轮询继续处理，而不会虚报全部完成。
8. 在主号详情或全部隐私邮箱页面查看归属、状态和最近收件时间，并从操作列复制邮件 API 直达链接。需要时可以停用、永久删除或轮换 Key。
9. 在操作记录页面检查登录、配置变更、目录同步、邮件同步和 Key 轮换记录。同步失败时，状态旁先显示简短错误；点击其后的“点击查看错误日志”可打开本次同步的完整错误日志。
10. 在“全部日志”页面按级别、关键词或主号筛选应用运行日志。点击带有同步编号或自动创建编号的记录，会分别按 `sync_run_id` 或 `auto_create_run_id` 精确聚合同一轮流程的全部可用分页，并按时间顺序展示阶段时间线；普通日志仍显示单条详情。

主号列表、主号详情和隐私邮箱列表会在页面可见时每 5 秒静默读取最新同步状态、时间和活动进度；切换到其他标签页时暂停，返回页面或窗口重新获得焦点时立即刷新。活动进度属于当前进程内状态，同步结束后不再返回；最终成功或错误状态仍保存到数据库。旧版回退管理页使用相同的刷新规则。

永久删除会先读取 Apple 完整目录并按地址取得远端标识：启用地址先停用，再从 iCloud 永久删除；远端已经不存在时按幂等成功处理。服务仅在远端删除得到确认后清除本地 API Key、邮件快照及关联记录。Apple 会话失效、服务错误或结果尚未确认时，本地记录会保留，重新连接 Apple 账户后可再次执行删除完成对账。

每次手动或定时自动邮件同步都会分配一个 `sync_run_id`。同一次逻辑同步为了追平积压而连续执行多个批次时，批次编号从 1 递增，但继续共用同一个 `sync_run_id`；下一次独立同步使用新编号。同步流程日志覆盖排队、等待资源、准备、连接、认证、扫描、读取、校验、保存和最终完成、失败或取消，并记录事件时间、触发方式、批次、百分比、累计耗时、源码位置和步骤上下文。失败事件还会给出失败阶段、操作位置和已脱敏的详细错误。详情页把这些记录按时间从早到晚显示为时间线，并可复制当前可用的完整流程。

每个被任务认领的自动创建计划槽都会分配一个 `auto_create_run_id`。流程日志依次记录准备、读取主号、检查容量、加载和验证 Apple 会话、核对转发目标、准备 Key、远端预留、保存候选、确认与对账、保存结果以及最终完成、失败或取消；失败记录还会给出错误代码、错误类别和受控 `cause_category`、失败阶段、操作位置、HTTP 状态、是否可重试、计划后续动作与下次运行时间，并明确 Apple 侧是否可能已经产生副作用、候选是否待确认以及计划状态是否写回。日志不会记录隐私邮箱地址、原始 API Key、Apple 会话、Cookie、令牌或凭据等敏感内容。流程日志是从本次部署后新产生的事件开始记录的，部署前已经存在的单条旧日志无法补回历史步骤。

“全部日志”使用当前应用进程内的有界环形缓冲区，最多保留最近 2000 条记录，同时还受总字节数限制；记录较大时，更早日志可能在达到 2000 条前被覆盖。服务重启后从空缓冲区重新开始，不作为持久审计记录；需要追踪管理员操作时仍应使用“操作记录”。流程详情分别通过 `GET /admin/api/v1/logs?sync_run_id=...` 或 `GET /admin/api/v1/logs?auto_create_run_id=...` 精确筛选并遍历当前环形窗口内的分页；如果较早的 `run_started` 或最终完成、失败、取消事件已经被覆盖，页面会明确提示当前只展示部分流程。密码、令牌、Cookie、会话和凭据等敏感结构化字段在进入日志缓冲区前统一脱敏，对应值显示为 `[REDACTED]`；页面和日志接口只读取这个已脱敏的缓冲区。

轮换 Key 会直接吊销旧 Key。永久删除会同时吊销该地址的原始 Key 和已复制的派生直达链接。Vue 管理端只会在创建或轮换 Key 的结果中显示一次完整 API Key；离开结果页后无法从数据库重建，因为数据库不保存原始 Key。未保存原始 Key 时需要再次轮换。

邮件 API 直达链接不需要保存原始 Key：服务端会为管理端生成一个仅用于按当前邮件一次性消费的紧凑接口派生凭据，并在主号详情和全部隐私邮箱列表的操作列提供复制按钮。派生凭据不写入数据库，也不能用于 Bearer 接口；它与当前 API Key 状态绑定，轮换 Key 会同时吊销此前复制的直达链接。成功打开链接会消费当前最新邮件并异步把主号收件箱中的该邮件标为已读；链接本身不会因一次访问失效，新 UID 到来后同一链接可再次消费，直到 Key 被轮换、主号或隐私邮箱被停用。
...
新登记的地址初始同步状态为 `pending`，直达链接会在下一轮主号级未读同步完成后按收件 API 规则查询数据库；同步完成前会返回 `503 SYNC_UNAVAILABLE`。新增地址、批量导入至少一个启用地址或重新启用地址会使所属主号的增量游标失效；下一轮按首次同步规则先确定收件箱最近最多 1024 封邮件的窗口边界，再仅处理窗口内的 `UNSEEN` 邮件并建立新的 `UIDVALIDITY + last_uid` 基线，不读取已读邮件正文，也不恢复各隐私邮箱的历史 IMAP 快照。
...
完整接口定义见 [`docs/openapi.yaml`](docs/openapi.yaml)。服务提供一个可直接访问、按当前数据库快照一次性消费的紧凑链接接口，以及一个使用 Bearer Header 的完整接口。两个接口都只查询 PostgreSQL 中当前保存的快照：紧凑接口仅在其 IMAP 收件时间位于 `[当前时间-1小时, 当前时间]` 且该 `UIDVALIDITY + UID` 从未被当前隐私邮箱消费时返回 `200`；服务会在返回成功前用同一个 PostgreSQL 事务原子记录消费并写入持久化的 IMAP `\Seen` 队列。同一 UID 的后续直达请求返回 `404 MAIL_NOT_FOUND`，直到同步到未消费的新 UID。iCloud 中删除当前邮件不会触发历史扫描或回退到次新邮件。完整 Bearer 接口只要同步状态仍新鲜就可重复返回已保存的最新快照，不限制邮件年龄，也不触发消费或已读标记。
...
该接口方便只能直接打开 URL 的调用方使用。`api_key` 必填且只能出现一次，只用于定位它绑定的唯一隐私邮箱；Bearer Header 不能替代这个查询参数，接口也不接受由调用方指定的邮箱地址。查询参数既兼容创建或轮换隐私邮箱时显示的完整 API Key，也接受管理端操作列复制的派生直达凭据；派生凭据不能反过来用于 Bearer 接口。对当前最新且位于一小时窗口内的同一 UID，并发请求中只有一个请求能完成原子消费并返回 `200`，其余请求返回 `404 MAIL_NOT_FOUND`。以下占位值故意不是有效 Key，实际使用时可直接在后台复制完整链接：
...
消费事务提交后，后台任务会按主号批量执行 `UID STORE ... +FLAGS.SILENT (\Seen)`。这个过程不阻塞直达接口，iCloud 的已读状态可能短暂延迟；IMAP 操作失败时任务会留在 PostgreSQL 中等待重试，不会撤销已经提交的消费。`\Seen` 是主号 `INBOX` 中这封邮件的全局状态，因此 Apple Mail、iCloud Mail 和其他 IMAP 客户端也会看到它变为已读。

查询参数会进入浏览器历史，并可能被反向代理或外围监控记录、通过 Referer 泄露。本应用内置 HTTP 日志只记录 URL 路径，不记录查询字符串，但不能代替外围系统的脱敏配置。完整链接必须和 API Key 一样按密钥保护。该接口使用具有副作用的 `GET`，浏览器预取、链接预览和安全扫描器都可能在实际访客之前消费邮件；不要把直达链接交给会自动访问 URL 的系统。消费事务在写入响应前提交，因此即使调用方超时、断线或没有收到响应，邮件也可能已经消费，重试会得到 `404`；需要可恢复读取时应使用 Bearer 接口。泄露链接即使当前只返回 `404`，仍可在未来新 UID 到来后消费新邮件。能设置请求头的客户端应优先使用下面的 Bearer 接口；确实使用紧凑链接时，只能通过 HTTPS 传输，并应在代理层关闭或脱敏查询字符串日志，不要把链接发到工单、聊天、分析平台或第三方页面。怀疑链接泄露后立即在后台轮换 Key。
...
此接口是可重复读取的只读快照接口：重复请求同一 `message.id` 仍会返回 `200`，不会写入一次性消费记录、不会创建 IMAP `\Seen` 任务，也不受紧凑接口是否已经消费同一 UID 的影响。
...
| `404` | `MAIL_NOT_FOUND` | 同步结果仍新鲜但没有邮件快照；紧凑接口还会在最新快照不处于 `[当前时间-1小时, 当前时间]` 或同一 `UIDVALIDITY + UID` 已被当前隐私邮箱消费时返回此错误，完整 Bearer 接口不限制邮件年龄且不受消费状态影响 |
...
| `503` | `DATABASE_UNAVAILABLE` | 查询 Key/最新邮件快照，或紧凑接口原子提交消费记录与 `\Seen` 队列时数据库暂不可用；事务失败不会产生部分消费 |

错误响应统一包含错误码、说明和请求编号。下面是紧凑接口在当前快照不属于最近一小时窗口或已被消费时的示例；为避免泄露消费状态，两种情况使用同一错误。完整 Bearer 接口没有邮件年龄或消费状态限制，且无快照时使用“尚未收到邮件”：
...
- 收件 API 的请求处理只查询或更新 PostgreSQL，不会等待建立 iCloud 连接；因此快照延迟通常为一个 `ICLOUD_API_POLL_INTERVAL`，外加 IMAP 网络耗时。直达接口提交消费后只通知异步 `\Seen` 工作器，响应不等待 iCloud 状态更新。
- 邮件发现与快照同步按部署前提工作：所有路径都只把主号 `INBOX` 中游标范围内的 `UNSEEN` UID 送入邮件头和正文读取；已读 UID 只用于推进主号游标，不进入正文读取。首次同步、游标重置或 `UIDVALIDITY` 变化没有已读回扫例外。邮件头和正文通过 `BODY.PEEK` 读取，同步读取本身不会设置 `\Seen`；Apple Web 会话只用于读取隐私邮箱完整目录，不参与邮件收取。不要让其他客户端或外部流程读取该 INBOX，否则邮件可能在本服务同步前变为已读并被跳过。
- 每轮邮件同步按主号建立一个只读 IMAP 连接，并处理这个主号下的所有已启用隐私邮箱，不会为每个隐私邮箱分别查询 iCloud。首次同步、游标重置或 `UIDVALIDITY` 变化时，先用少量 sequence UID 查询确定最近最多 1024 封邮件的窗口边界并提交该边界，后续批次再走相同的主号级 `UNSEEN` 发现流程；确定边界时不读取任何邮件头或正文。每批最多提交 256 封未读邮件；较小的 UID 数值区间使用有界 `UID SEARCH ... UNSEEN`，较大或稀疏的区间先用少量序号探测定位起点，再用一次最多返回 257 条的 sequence `FETCH (UID FLAGS)` 检查窗口并在本地排除 `\Seen`。只有其中最早的最多 256 封未读邮件会进入后续邮件头/正文 FETCH。候选邮件头按 UID 批量 `BODY.PEEK` 读取并在本地归类，只对各地址最终命中的最新候选读取完整正文；正文每条命令最多处理 8 封，避免网络传输与 MIME 解析共同耗尽同一条 IMAP 命令的截止时间。只读同步遇到 EOF、连接重置或 `imap: connection closed` 时，会在短暂等待后新建连接重试一次。单封邮件的投递头损坏、超限或相互冲突时，只跳过该封不可安全归属的邮件，不会阻塞整个主号的游标；IMAP 命令失败、重复 UID 或邮箱序号视图异常仍会让本批失败并从已提交游标重试。
- 定时同步和后台手动同步都会先取得该主号的账号锁及全局 IMAP 连接额度，随后才开始计算 `ICLOUD_API_SYNC_TIMEOUT`；默认 `10m` 因此是实际执行数据库读取、IMAP 收取和事务提交的完整预算，不会消耗在排队等待上。每条 IMAP 网络命令仍分别受默认 `25s` 的 `ICLOUD_API_IMAP_TIMEOUT` 限制。页面发起手动同步时只把主号加入去重的后台队列并立即返回 HTTP `202`；同一主号的重复请求合并，积压批次会在后台连续追平。手动与定时自动同步都会发布当前来源、阶段和完成百分比，页面通过状态刷新实时显示进度与最终结果；流程事件同时以同一个 `sync_run_id` 聚合，跨批追平不会拆成互不关联的多段日志。
- PostgreSQL 为每个主号保存 `UIDVALIDITY + last_uid`，并为每个隐私邮箱保存当前数据库快照。`UIDVALIDITY` 未变化时从 `last_uid + 1` 开始处理；同步只校验本批刚读取的未读候选，不读取或校验每个隐私邮箱的历史 IMAP 快照。iCloud 邮件被删除不会触发次新邮件回退；数据库快照只会被后续未读同步结果替换，或在邮箱代际变化及显式清理时删除。
- 首次同步、游标重置或 `UIDVALIDITY` 变化时会先建立最近最多 1024 封邮件的窗口边界，再从窗口最旧端开始按主号级 `UNSEEN` 分批推进。1024 只限制候选窗口，不代表读取 1024 封正文；已读邮件不会进入邮件头或正文 FETCH。UID 即使因删除邮件而不连续，也不会缩小按实际邮件序号确定的窗口。
- 未读积压超过 256 封时，每轮从最早尚未处理的未读邮件开始处理最多 256 封，并在同一 PostgreSQL 事务中提交本批邮件快照和批末游标；后续轮次从该游标继续追平，已经由本服务标记为已读的 UID 会跳过。追平期间主号及其启用隐私邮箱保持 `pending`，收件 API 返回 `503`；全部追平后才标记为 `ok`。首次和重置路径在提交窗口边界后也使用同一个 256 封未读批次上限。
- 新建隐私邮箱、批量导入至少一个启用项、重新启用隐私邮箱、更新主号 IMAP 凭据、重新启用主号或执行整主号快照重置，都会使主号游标失效；下一轮只重建主号级最近窗口游标并处理 `UNSEEN`，不会为各隐私邮箱恢复历史快照。同一 `UIDVALIDITY` 下未被本批结果替换的已有数据库快照保持不变；邮箱代际真正变化时才清理旧代快照。
- 邮件快照、主号游标和同步状态在同一个 PostgreSQL 事务中提交；任一步失败都不会提前推进游标。一次主号同步失败会把该主号及其已启用地址标为 `error`，同时保存面向列表展示的简短摘要和可通过“点击查看错误日志”打开的完整错误日志，其他主号不受影响；积压批次的成功提交仍保持 `pending`，不属于同步失败。
...
- PostgreSQL 对每个隐私邮箱只保留一行由未读同步写入的当前快照。新的未读 UID 会替换旧 UID；iCloud 删除邮件不会触发历史扫描或次新回退，接口也不会为了凑足一小时窗口而选择更旧邮件。`consumed_messages` 按隐私邮箱严格保留所有已通过紧凑接口消费的 `(UIDVALIDITY, UID)` 历史，确保同一快照只消费一次；`UIDVALIDITY` 把不同邮箱代际的相同 UID 隔离开。`imap_seen_tasks` 持久保存尚待写回 iCloud 的主号级 `\Seen` 任务。
- `pending` 表示地址刚创建、重新启用、因主号凭据变化而重置，或主号仍在分批追平积压；收件 API 返回 `503`。手动同步请求进入后台队列后返回 HTTP `202`，页面继续刷新主号与隐私邮箱状态。
- `error` 表示最近一次主号级未读同步失败。旧快照可能仍保存在内部，但 API 会返回 `503`，不会把它作为可用邮件返回。
- `ok` 表示最近一次主号级未读同步已追平，API 可以按数据库当前快照作答；它不表示服务逐个检查过隐私邮箱的全部 IMAP 历史。在三个轮询周期的有效期内，完整 Bearer 接口有快照就可重复返回 `200`、没有快照时返回 `404`；紧凑接口仅在当前快照的 IMAP 收件时间处于 `[当前时间-1小时, 当前时间]` 且该 UID 尚未消费时返回一次 `200`，同一 UID 的后续请求返回 `404`，直到新的未读 UID 替换当前快照。最后同步时间晚于当前时间或结果超过有效期时，两个接口都返回 `503`。
- 当前版本的成功响应中 `stale` 始终为 `false`。降级状态统一返回 `503`，不会返回带 `stale=true` 的旧快照。
- 使用完整接口的客户端可保存 `message.id` 并定时重试；ID 未变化时由客户端决定是否重复处理。紧凑接口由服务端按 `UIDVALIDITY + UID` 保证一次性消费，成功后轮询会持续得到 `404`，直到同步到新邮件。建议轮询频率低于服务端每分钟 120 次的 Key 限流。

## 备份与恢复

当前业务数据位于 `postgres_data`，其中包括自动创建计划、待领取 Key、一次性消费记录和尚未完成的 IMAP `\Seen` 队列。schema v6 备份必须完整保留 `alias_creation_schedules`、`pending_alias_api_keys`、`consumed_messages` 和 `imap_seen_tasks`，否则可能丢失自动创建进度或一次性凭据、让旧 UID 被直达接口再次消费，或丢失已经消费但尚未写回 iCloud 的已读任务。升级前生成的 schema v4/v5 归档仍可恢复：正式 schema v5 归档中的自动创建计划和待领取 Key 会原样保留，迁移到 v6 时创建空的消费历史和 IMAP `\Seen` 队列，因此已有最新快照初始视为尚未消费；早期只包含消费历史和 IMAP `\Seen` 队列的 v5 归档也会保留这两类数据，并创建空的自动创建计划和待领取 Key 表。v4 归档会依次迁移到 v5 和 v6，四张扩展表均从空表开始。为保证 PostgreSQL 与 `icloud_api_keys` 属于同一恢复点，逻辑备份也应在维护窗口中执行：先取得固定名维护锁，停止应用并保持 PostgreSQL 运行，在同一个临时目录中生成并校验两份归档，再以一个版本化目录发布恢复集；应用通过健康检查后才释放维护锁。镜像内置的数据库包装会从 `postgres_config` 动态读取随机数据库名、用户和密码，在容器内部完成 SCRAM 认证；数据库端口无需发布到宿主机。任一备份、校验、发布或重新启动步骤失败时，下面的命令都会保持应用停止：
...
PostgreSQL `restore` 会动态读取目标安装新生成或已有的随机数据库名、用户和密码，并接受 stdin 或一个归档路径。它先把输入保存到权限受限的临时文件，检查 `PGDMP` 标识和非空归档目录，并要求 schema v4/v5/v6 共有的 11 张项目表、每张表的 `TABLE DATA` 段、四个身份序列及其 `SEQUENCE SET` 状态，以及共有的主键、唯一约束、外键和八个关键查询索引都存在。正式 schema v5 归档还必须同时包含 `alias_creation_schedules`、`pending_alias_api_keys` 及其数据段、约束、外键和第九个计划队列索引；早期 Seen-only v5 归档则必须同时包含 `consumed_messages`、`imap_seen_tasks` 及其数据段、复合主键、级联外键和第九个 Seen 队列索引。schema v6 归档必须完整包含这四张扩展表和两个扩展索引。之后 `pg_restore --clean --if-exists --no-owner --no-acl --exit-on-error` 会把整份 custom archive 成功展开为 SQL，这些预检步骤都不会连接或修改目标数据库。内建 `backup` 不接受改变 dump 范围或格式的附加参数，因此不会生成可误恢复的 `schema-only` 或缺少 post-data 的归档。验证完成后，同一个 `psql --single-transaction` 会依次重建 `public` schema、执行恢复 SQL、读取 `schema_migrations` 并只接受 v4、三种已知完整布局的 v5 或 v6：v4 归档必须不含扩展表；v5 归档可以完整包含自动创建表组、Seen 表组或两组同时存在，但任一表组只有一张表都会被拒绝；v6 归档必须完整包含两组扩展表。v4/v5 归档在应用下次启动时会原子迁移到 v6。各版本所需结构都会通过 `pg_catalog` 校验。事务最后会再次收紧 schema 权限；`ON_ERROR_STOP` 会让任意 SQL 或结构断言错误回滚整个事务。因此空归档、损坏归档、项目对象、数据段或 post-data 缺失和展开失败不会改变现有数据，有效归档在执行阶段失败也会保留恢复前的完整 schema。数据库备份与恢复共用独立的互斥锁，宿主维护窗口另由 `icloud-api-maintenance-lock` 覆盖全生命周期；归档由本项目的 `backup` 生成且不包含 `CREATE DATABASE`，恢复始终写入目标安装当前的随机数据库，不使用 `pg_restore --create`。
...
恢复后登录后台并验证管理员凭据、OAuth 调用、主号凭据解密、IMAP 同步、邮件 API 和审计记录；还应确认已消费 UID 的紧凑接口仍返回 `404`、完整 Bearer 接口仍可重复读取同一快照，且恢复的待处理 `\Seen` 任务会继续执行。原生部署同样应使用 `pg_dump`/`pg_restore` 备份 PostgreSQL，并同时保存 `ICLOUD_API_MASTER_KEY_FILE` 或密钥管理系统中的 `ICLOUD_API_MASTER_KEY`。

## 安全事项

- 生产环境应放在 HTTPS 反向代理之后，并设置 `ICLOUD_API_COOKIE_SECURE=true`。限制后台来源 IP，不要直接把无 TLS 的管理端口暴露到公网。
- `.env`、`postgres_config`、`icloud_api_keys`、OAuth Token、App 专用密码、受信任 Apple Web 会话、完整 API Key、PostgreSQL 数据/备份、旧 SQLite 数据和主密钥都属于敏感信息。限制文件/卷权限，不要写入代码、工单或普通访问日志。Apple 账户密码和双重认证验证码仅在连接流程中使用，不落库；会话过期后重新登录，不要尝试保存或复用验证码。外部登记接口必须通过 HTTPS 调用，怀疑 OAuth Token 泄露后应立即轮换并重启服务。`/api/v1/mail/recent` 为支持直接访问而有意把 Key 放入 URL，并会在成功的 `GET` 请求中消费邮件、异步改变主号全局 `\Seen` 状态；浏览器预取、链接预览或安全扫描可能提前触发这些副作用。应优先使用不在 URL 暴露 Key、可重复读取且不触发消费的 `/api/v1/mail/latest`；使用紧凑链接时必须把整个 URL 视为密钥并对代理日志中的查询字符串做删除或脱敏。
...
- 生产环境维持单个 `icloud-api` 副本运行。多个实例会重复轮询 IMAP、重复尝试处理同一批 `\Seen` 任务，并拆分进程内同步互斥与限流状态。
