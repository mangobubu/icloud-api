...
这是一个使用 Go、Gin、Vue 3、Element Plus、iCloud IMAP 和 SQLite 构建的轻量收件服务。Vue 管理端可以登记 iCloud 主号，并为主号绑定多个“隐藏邮件地址”（隐私邮箱）；Go API 服务定时通过该主号的同一个 IMAP 收件箱同步邮件。每个隐私邮箱拥有独立 API Key，调用者只能读取这个 Key 所属隐私邮箱的最新邮件快照；可直接访问的紧凑链接只在该快照属于最近一小时时返回邮件。
...
轮换 Key 会直接吊销旧 Key。Vue 管理端只会在创建或轮换 Key 的结果中显示一次完整 Key 和完整紧凑链接；离开结果页后无法从数据库重建，因为数据库不保存原始 Key。未保存时需要再次轮换。
...
```http
GET /api/v1/mail/recent?api_key=icm_<后台显示的43位RawBase64URL密钥>
```

该接口方便只能直接打开 URL 的调用方使用。`api_key` 必填且只能出现一次，只用于定位它绑定的唯一隐私邮箱；Bearer Header 不能替代这个查询参数，接口也不接受由调用方指定的邮箱地址。以下占位值故意不是有效 Key，调用时应替换为后台创建或轮换隐私邮箱时显示的完整 Key：
...
`email` 是 Key 绑定的隐私邮箱；`content` 优先返回解析后的纯文本正文，纯文本为空时回退到 HTML 正文，并可能因服务端邮件或正文大小上限而截断；`time` 是 IMAP 服务器记录的收件时间，按 `.env` 中 `TZ` 对应的时区格式化为 RFC 3339。上例使用 `TZ=Asia/Shanghai`，因此偏移为 `+08:00`。时区只改变显示形式，不改变“最近一小时”的绝对时间窗口。
...
- API 返回的 `html` 是外部邮件内容；紧凑接口在纯文本为空时，`content` 也可能是 HTML。前端展示这些内容时必须做 HTML 清理或放入严格隔离的沙箱，禁止直接插入管理页面 DOM。
