...
这是一个使用 Go、Gin、Vue 3、Element Plus、iCloud IMAP 和 PostgreSQL 构建的轻量收件服务。Vue 管理端可以登记 iCloud 主号，并为主号绑定多个“隐藏邮件地址”（隐私邮箱）；Go API 服务定时通过该主号的同一个 IMAP 收件箱同步邮件。每个隐私邮箱拥有独立 API Key，调用者只能读取这个 Key 所属隐私邮箱的最新邮件快照；可直接访问的紧凑链接会一次性消费最近一小时内的当前最新邮件，完整 Bearer 接口则可重复读取快照。
...
- 邮件同步以主号为单位建立一条只读 IMAP 连接，并使用 PostgreSQL 中保存的 `UIDVALIDITY + last_uid` 游标读取新增 UID；隐私邮箱数量不会转换成同等数量的 iCloud 查询。直达接口成功消费邮件后，独立后台任务再按主号建立可写 IMAP 连接，把对应 UID 标记为 `\Seen`。
...
App 专用密码只用于后台的 IMAP 密码字段。邮件正文和文件夹始终通过 `imap.mail.me.com:993` 和 TLS 获取：快照同步使用只读 `SELECT`，普通增量按主号级有界窗口检查游标后的 UID/FLAGS 并只读取未读邮件，通过 `BODY.PEEK` 读取时不会设置已读；首次同步、`UIDVALIDITY` 变化和最近窗口回退仍扫描窗口内实际邮件。只有直达接口成功消费后的后台任务才使用可写 `SELECT` 和 `UID STORE` 设置 `\Seen`。该部署假定没有其他邮件客户端或外部流程读邮件/改变 `\Seen`；IMAP 用户名通常是完整的 iCloud 主号邮箱地址。Apple 账户密码不会代替 App 专用密码访问邮件。
...
完整接口定义见 [`docs/openapi.yaml`](docs/openapi.yaml)。服务提供一个可直接访问、按当前邮件一次性消费的紧凑链接接口，以及一个使用 Bearer Header 的完整接口。两个接口都以当前最新快照为准：紧凑接口仅在其 IMAP 收件时间位于 `[当前时间-1小时, 当前时间]` 且该 `UIDVALIDITY + UID` 从未被当前隐私邮箱消费时返回 `200`；服务会在返回成功前用同一个 PostgreSQL 事务原子记录消费并写入持久化的 IMAP `\Seen` 队列。同一 UID 的后续直达请求返回 `404 MAIL_NOT_FOUND`，直到同步到未消费的新 UID；删除或回退后重新成为最新快照的已消费旧 UID 仍返回 `404`。完整 Bearer 接口只要同步状态仍新鲜就可重复返回已保存的最新快照，不限制邮件年龄，也不触发消费或已读标记。
...
- 邮件发现与快照同步按部署前提工作：普通增量只把主号 `INBOX` 中游标后的未读 UID 送入邮件头和正文读取；已读 UID 会被纳入游标范围并跳过，空候选也会把游标推进到当前上界。首次同步、`UIDVALIDITY` 变化以及删除后的最近窗口回退仍扫描窗口内实际邮件，不依赖未读标记。邮件头和正文通过 `BODY.PEEK` 读取，同步读取本身不会设置 `\Seen`；Apple Web 会话只用于读取隐私邮箱完整目录，不参与邮件收取。不要让其他客户端或外部流程读取该 INBOX，否则已读邮件可能在本服务同步前被跳过。
- 每轮邮件同步按主号建立一个只读 IMAP 连接，并处理这个主号下的所有已启用隐私邮箱，不会为每个隐私邮箱分别查询 iCloud。普通增量每批最多提交 256 封，提交后立即继续下一批；较小的 UID 数值区间使用有界 `UID SEARCH ... UNSEEN`，较大或稀疏的区间先用少量序号探测定位起点，再用一次最多返回 257 条的 sequence `FETCH (UID FLAGS)` 检查窗口并在本地排除 `\Seen`。只有其中最早的最多 256 封未读邮件会进入后续邮件头/正文 FETCH。候选邮件头按 UID 批量 `BODY.PEEK` 读取并在本地归类，只对各地址最终命中的最新候选读取完整正文；正文每条命令最多处理 8 封，避免网络传输与 MIME 解析共同耗尽同一条 IMAP 命令的截止时间。只读同步遇到 EOF、连接重置或 `imap: connection closed` 时，会在短暂等待后新建连接重试一次。单封邮件的投递头损坏、超限或相互冲突时，只跳过该封不可安全归属的邮件，不会阻塞整个主号的游标；IMAP 命令失败、重复 UID 或邮箱序号视图异常仍会让本批失败并从已提交游标重试。
...
- PostgreSQL 为每个主号保存 `UIDVALIDITY + last_uid`，并保存每个隐私邮箱当前快照的 UID 位置。`UIDVALIDITY` 未变化时从 `last_uid + 1` 开始处理；追平新增邮件后，服务用一条共享 `UID FETCH` 校验所有已保存的最新 UID。若邮件已从 iCloud 删除，服务会共享扫描收件箱中实际存在的最新最多 1024 封邮件，为受影响地址回退到该窗口内的次新邮件；没有可用回退时删除本地快照。整个过程仍是主号级批量操作，不会产生逐隐私邮箱查询。
- 首次同步或 `UIDVALIDITY` 变化时会建立新游标、重置旧快照，并且只扫描收件箱中实际存在的最新最多 1024 封邮件，不会全量下载历史邮件。UID 即使因删除邮件而不连续，也不会因此缩小实际邮件窗口。
- 正常增量积压超过 256 封未读邮件时，每轮从最早尚未处理的未读邮件开始处理最多 256 封，并在同一 PostgreSQL 事务中提交本批邮件快照和批末游标；后续轮次从该游标继续追平，已经由本服务标记为已读的 UID 会安全跳过。追平期间主号及其启用隐私邮箱保持 `pending`，收件 API 返回 `503`；全部追平后才标记为 `ok`。这个 256 封上限只用于普通增量批次，首次同步、`UIDVALIDITY` 变化、配置重置和上述删除回退仍使用最新最多 1024 封的有界窗口。
...
- PostgreSQL 对每个隐私邮箱只保留一行最新邮件。新 UID 会替换旧 UID；当前最新邮件被删除时，只会在上述有界最近窗口内寻找回退，不会把 PostgreSQL 变成邮件归档库。接口也不会为了凑足一小时窗口而回退到更旧邮件。`consumed_messages` 按隐私邮箱严格保留所有已通过紧凑接口消费的 `(UIDVALIDITY, UID)` 历史，避免邮件删除或快照回退后重复消费旧 UID；`UIDVALIDITY` 把不同邮箱代际的相同 UID 隔离开。`imap_seen_tasks` 持久保存尚待写回 iCloud 的主号级 `\Seen` 任务。
...
- `ok` 表示最近同步已明确找到最新邮件，或权威确认当前为空。在三个轮询周期的有效期内，完整 Bearer 接口有快照就可重复返回 `200`、没有快照时返回 `404`；紧凑接口仅在最新快照的 IMAP 收件时间处于 `[当前时间-1小时, 当前时间]` 且该 UID 尚未消费时返回一次 `200`，同一 UID 的后续请求返回 `404`，直到新 UID 替换当前快照。最后同步时间晚于当前时间或结果超过有效期时，两个接口都返回 `503`。
