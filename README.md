...
主号详情页的“自动创建隐私邮箱”开关按主号独立生效。开启后，服务以开启时刻为周期锚点，每 60 分钟最多发起 5 次 Apple Hide My Email 创建尝试；每次间隔随机生成且至少 5 分钟，5 段间隔合计恰好 60 分钟。计划保存在数据库，服务重启后继续使用原计划；任务严重迟到时会从当前时间重新排期，不会一次性补发。Apple 会话失效、限流或服务故障只消耗当前计划槽，不会立即重试。

自动创建的地址会先写入本地主号并生成独立邮件 API Key。原始 Key 使用主密钥的独立加密上下文进入“待领取”队列，不会写入日志；管理员在主号详情页领取后，应下载或保存 Key，再点击确认保存。只有本次领取返回的地址 ID 会被删除，之后新产生的待领取 Key 不会被误删。达到每个主号 1000 个启用隐私邮箱上限后，服务停止该主号的自动创建。

开启前主号必须已有有效的 Apple Web 会话，且 Apple 隐私邮箱转发目标必须是当前主号。每个计划槽都会在远端创建前重新读取 Apple 的 `selectedForwardTo`；目标不匹配或无法确认时不会调用 `generate`/`reserve`。`reserve` 省略转发信息时，服务还会通过远端列表按新地址确认实际转发目标，避免把创建前的设置快照误当作创建结果。自动创建与手动“同步隐私邮箱”共用主号操作锁；外部 `POST /api/v1/aliases` 仍只登记已经存在的地址，不会替代自动创建流程。一个隐私邮箱只能登记一次，也只能归属于一个主号。
