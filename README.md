...
10. 在“全部日志”页面按级别、关键词或主号筛选应用运行日志。点击带有同步编号的记录会按 `sync_run_id` 精确聚合同一轮同步的全部可用分页，并按时间顺序展示阶段时间线；普通日志仍显示单条详情。
...
“全部日志”使用当前应用进程内的有界环形缓冲区，最多保留最近 2000 条记录，同时还受总字节数限制；记录较大时，更早日志可能在达到 2000 条前被覆盖。服务重启后从空缓冲区重新开始，不作为持久审计记录；需要追踪管理员操作时仍应使用“操作记录”。同步详情通过 `GET /admin/api/v1/logs?sync_run_id=...` 精确筛选并遍历当前环形窗口内的分页；如果较早的 `run_started` 或最终完成、失败、取消事件已经被覆盖，页面会明确提示当前只展示部分流程。密码、令牌、Cookie、会话和凭据等敏感结构化字段在进入日志缓冲区前统一脱敏，对应值显示为 `[REDACTED]`；页面和日志接口只读取这个已脱敏的缓冲区。
