...
10. 在“全部日志”页面按级别、关键词或主号筛选应用运行日志，结合请求编号和结构化属性定位具体失败环节。
...
“全部日志”只在内存中保留当前应用进程最近最多 2000 条记录，服务重启后从空日志缓冲区重新开始，不作为持久审计记录；需要追踪管理员操作时仍应使用“操作记录”。密码、令牌、Cookie、会话和凭据等敏感结构化字段在进入日志缓冲区前统一脱敏，对应值显示为 `[REDACTED]`；页面和 `GET /admin/api/v1/logs` 只读取这个已脱敏的缓冲区。
...
- 定时同步和后台手动同步都会先取得该主号的账号锁及全局 IMAP 连接额度，随后才开始计算 `ICLOUD_API_SYNC_TIMEOUT`；默认 `10m` 因此是实际执行数据库读取、IMAP 收取和事务提交的完整预算，不会消耗在排队等待上。每条 IMAP 网络命令仍分别受默认 `25s` 的 `ICLOUD_API_IMAP_TIMEOUT` 限制。页面发起手动同步时只把主号加入去重的后台队列并立即返回 HTTP `202`；同一主号的重复请求合并，积压批次会在后台连续追平。手动与定时自动同步都会发布当前来源、阶段和完成百分比，页面通过状态刷新实时显示进度与最终结果。
